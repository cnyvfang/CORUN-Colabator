...
from data.transforms import solo_random_crop, triple_random_crop
...
class HazeOnlineDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'RIDCP' or 'LAP' in self.depth_folder:
            depth_path = os.path.join(self.depth_folder, gt_path.split('/')[-1].split('.')[0] + '.npy')
            img_depth = np.load(depth_path)
            if 'LAP' in self.depth_folder:
                img_depth = img_depth.squeeze()
            img_depth = (img_depth - img_depth.min()) / (img_depth.max() - img_depth.min())
        else:
            depth_path = os.path.join(self.depth_folder, gt_path.split('/')[-1].split('.')[0] + '.mat')
            img_depth = mat73.loadmat(depth_path)['depth']
        ...
        if self.opt.get('use_erode_t', False):
            if np.random.rand(1) < 0.5:
                iter = int(np.random.randint(1, 3))
                t = cv2.erode(t, np.ones((3, 3), np.uint8), iterations=iter)
        # add haze
        ...
        t = torch.from_numpy(t).unsqueeze(0).float()
...
@DATASET_REGISTRY.register()
class SemiHazeOnlineDataset(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(SemiHazeOnlineDataset, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        ...
        self.gt_paths = make_dataset(self.gt_folder)
        self.depth_paths = make_dataset(self.depth_folder)
        self.real_paths = make_dataset(self.real_folder)
        self.beta_range = opt['beta_range']
        self.A_range = opt['A_range']
        self.color_p = opt['color_p']
        self.color_range = opt['color_range']
    ...
    def __getitem__(self, index):
        ...
        gt_path = self.gt_paths[index % len(self.gt_paths)]
        real_path = self.real_paths[index % len(self.real_paths)]
        img_gt = cv2.imread(gt_path).astype(np.float32) / 255.0
        img_real = cv2.imread(real_path).astype(np.float32) / 255.0
        ...
        if np.random.rand(1) < 0.5:
            img_lq = add_JPEG_noise(img_lq)
            img_real = add_JPEG_noise(img_real)
    ...
    def __len__(self):
        if len(self.gt_paths) >= len(self.real_paths):
            return len(self.gt_paths)
        else:
